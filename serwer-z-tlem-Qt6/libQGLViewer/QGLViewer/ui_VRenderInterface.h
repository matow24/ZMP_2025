...
public:
    ...
    void setupUi(QDialog *VRenderInterface)
    {
        if (VRenderInterface->objectName().isEmpty())
            VRenderInterface->setObjectName("VRenderInterface");
        ...
        tightenBBox->setObjectName("tightenBBox");
        ...
        QObject::connect(SaveButton, &QPushButton::released, VRenderInterface, qOverload<>(&QDialog::accept));
        QObject::connect(CancelButton, &QPushButton::released, VRenderInterface, qOverload<>(&QDialog::reject));
